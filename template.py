...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "Evaluate the response using each rubric criterion. "
        ...
        try:
            ...
            candidate_scores = parsed.get("scores", parsed) if isinstance(parsed, dict) else {}
            scores: dict[str, float] = {}
            for criterion in rubric:
                value = candidate_scores.get(criterion, 0.5)
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for index, result in enumerate(scores_batch):
            ...
            if index % 2 == 0:
                first_scores.extend(numeric)
            else:
                later_scores.extend(numeric)
        ...
        if first_scores and later_scores:
            ...
            positional_bias = first_average - later_average > 0.1
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            suggestion = suggestions[index - 1] if index <= len(suggestions) else "Review and improve the evaluation pipeline"
            values = (
                f"F{index:03d}",
    ...
    def generate_improvement_suggestions(
        ...
        actions = {
            "hallucination": "Add grounding checks and require every answer claim to be supported by retrieved context",
            "irrelevant": "Clarify the system prompt and add intent focused examples for direct answers",
            "incomplete": "Improve retrieval coverage and provide examples of complete multi part answers",
            "off_topic": "Add intent classification and reject context that does not match the user question",
            "refusal": "Review safety guardrails and add permitted domain examples to reduce false refusals",
        ...
        for failure_type, _ in sorted(categories.items(), key=lambda item: item[1], reverse=True):
            suggestions.append(actions.get(failure_type.lower(), actions["unknown"]))

        general_actions = [
            "Add regression tests for every confirmed failure before applying a fix",
            "Track answer and retrieval metrics separately to isolate the failing pipeline stage",
            "Expand the golden dataset with reviewed edge cases from the failure clusters",
        ]
